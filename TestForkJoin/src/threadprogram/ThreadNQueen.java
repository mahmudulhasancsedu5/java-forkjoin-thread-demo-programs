...
public class ThreadNQueen extends Thread {
	...
	public static void main(String[] args) {
		...
		try {
			...
			try {
				...
				System.out.println("Usage: java ThreadNQueen <threads> <boardSize>");
			...
			ThreadNQueen boardGame = new ThreadNQueen(new int[0]);
			boardGame.start();
			boardGame.join();
			...
			if (row >= boardSize) // done
				...
				for (int q = 0; q < boardSize; ++q) {
					...
					if (!attacked) {
						...
						ThreadNQueen game = new ThreadNQueen(next);
						game.start();
						try {
							game.join();
